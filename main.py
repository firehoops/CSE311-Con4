...
class Controller:
    ...
    def gui(self, master):
        ...
        Button(bottomFrame, text="Row 1", command=lambda: \
            self.addPiece(self.c, colTracker[0])).grid(row=9, column=0, sticky=E)
        Button(bottomFrame, text="Row 2", command=lambda: \
            self.addPiece(self.c, colTracker[1])).grid(row=9, column=1, sticky=E)
        Button(bottomFrame, text="Row 3", command=lambda: \
            self.addPiece(self.c, colTracker[2])).grid(row=9, column=2, sticky=E)
        Button(bottomFrame, text="Row 4", command=lambda: \
            self.addPiece(self.c, colTracker[3])).grid(row=9, column=3, sticky=E)
        Button(bottomFrame, text="Row 5", command=lambda: \
            self.addPiece(self.c, colTracker[4])).grid(row=9, column=4, sticky=E)
        Button(bottomFrame, text="Row 6", command=lambda: \
            self.addPiece(self.c, colTracker[5])).grid(row=9, column=5, sticky=E)
        Button(bottomFrame, text="Row 7", command=lambda: \
            self.addPiece(self.c, colTracker[6])).grid(row=9, column=6, sticky=E)

        Button(bottomFrame, text="Exit", command=lambda: self.quit(master)).grid(row=9, column=7, sticky=E)
        Button(bottomFrame, text="Switch Views", command=lambda: self.switchToText(master)).grid(row=9, column=8,
                                                                                                 sticky=E)

    # Adds a piece to the board
    # Params:Canvas, Boolean player value, col which was selected
    # Returns: Creates a piece on the board
    def addPiece(self, canvas, col):
        # Starting coord for row 1 , x0 =5 , y0 = 500 , x1 = 105 , y1= 600
        if col == 0:
            if model.playerValue == 1:
                canvas.create_oval(self.coords_col_1[0], self.coords_col_1[1], self.coords_col_1[2],
                                   self.coords_col_1[3], fill="black")
                ...
                canvas.create_oval(self.coords_col_1[0], self.coords_col_1[1], self.coords_col_1[2],
                                   self.coords_col_1[3], fill="red")
                self.coords_col_1[1] -= 100
                self.coords_col_1[3] -= 100
                model.makeMove(col)
        if col == 1:
            if model.playerValue == 1:
                canvas.create_oval(self.coords_col_2[0], self.coords_col_2[1], self.coords_col_2[2],
                                   self.coords_col_2[3], fill="black")
                ...
                canvas.create_oval(self.coords_col_2[0], self.coords_col_2[1], self.coords_col_2[2],
                                   self.coords_col_2[3], fill="red")
                self.coords_col_2[1] -= 100
                self.coords_col_2[3] -= 100
                model.makeMove(col)
        if col == 2:
            if model.playerValue == 1:
                canvas.create_oval(self.coords_col_3[0], self.coords_col_3[1], self.coords_col_3[2],
                                   self.coords_col_3[3], fill="black")
        ...
        if col == 3:
            if model.playerValue == 1:
                canvas.create_oval(self.coords_col_4[0], self.coords_col_4[1], self.coords_col_4[2],
                                   self.coords_col_4[3], fill="black")
        ...
        if col == 4:
            if model.playerValue == 1:
                canvas.create_oval(self.coords_col_5[0], self.coords_col_5[1], self.coords_col_5[2],
                                   self.coords_col_5[3], fill="black")
        ...
        if col == 6:
            if model.playerValue == 1:
                canvas.create_oval(self.coords_col_7[0], self.coords_col_7[1], self.coords_col_7[2],
                                   self.coords_col_7[3], fill="black")
